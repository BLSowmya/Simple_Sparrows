...
def contact(request):
    if request.method == 'POST':
        ...
        subject = 'RemoteCollab - New Enquiry'
...
def org_register(request):
    if request.method == "POST":
        ...
        if password1 == password2:
            ...
            subject = 'RemoteCollab - OTP Verification'
            message = f'Hi {o_name}, thank you for registering in RemoteCollab . Your One Time Password (OTP) for verfication is {otp}'
            email_from = settings.EMAIL_HOST_USER
...
def org_change_password(request):
    if request.method == 'POST':
        ...
        if org_details:
            subject = 'RemoteCollab - Password Changed'
            message = f'Hi, your password was changed successfully! From RemoteCollab'
            email_from = settings.EMAIL_HOST_USER
            recipient_list = [o_email, ]
            send_mail(subject, message, email_from, recipient_list)
            ...
            subject = 'RemoteCollab - Notifications'
            message = f'Hi, there was attempt to change your password! From RemoteCollab'
            email_from = settings.EMAIL_HOST_USER
            recipient_list = [o_email, ]
            send_mail(subject, message, email_from, recipient_list)
...
def org_forgot_password(request):
    if request.method == 'POST':
        ...
        if org_details:
            ...
            subject = 'RemoteCollab - OTP Verification for Forgot Password'
...
def org_forgot_password_change_password(request):
    if request.method == 'POST':
        ...
        if(pwd1 == pwd2):
            ...
            if org_details:
                subject = 'RemoteCollab - Password was Changed'
...
def report_org(request):
    if request.method == 'POST':
        ...
        recipient_list = ["narender.rk10@gmail.com",
                          ...
                          "2021.chinmay.vyapari@ves.ac.in"]
        send_mail(subject, message, email_from, recipient_list)
        send_mail(subject, "Your Problem has been recorded. From: RemoteCollab", email_from, [cemail])
...
def add_emp(request):
    if request.method == 'POST':
        ...
        if empObj:
            subject = 'RemoteCollab - Login Info'
        ...
        if projAssignCheck:
            ...
            if projAssignCheckObj:
                ...
                if user_details and project_details:
                    subject = 'RemoteCollab - Project was Assigned to you'
                    message = f'Hi, {s_name} You are asssigned to project {s_p_name} Check on RemoteCollab !'
                    email_from = settings.EMAIL_HOST_USER
        ...
        if meetingObj:
            ...
            if user_details:
                for ud in user_details:
                    subject = 'RemoteCollab - New Meeting'
                    s_name = ud['e_name']
                    s_email = ud['e_email']
                    message = f'Hi, {s_name} New Meeting for created for you! Details are Meeting Name :{m_name}, Meeting Description: {m_desc}, Date Time: {start_date} {start_time} Check on RemoteCollab !'
                    email_from = settings.EMAIL_HOST_USER
                    recipient_list = [s_email]
                    send_mail(subject, message, email_from, recipient_list)
            ...
            if meet_details:
                ...
                if user_details:
                    for ud in user_details:
                        ...
                        s_name = ud['e_name']
                        s_email = ud['e_email']
                        message = f'Hi, {s_name} New Meeting for created for you! Details are Meeting Name :{meet_name}, Meeting Description: {meet_desc}, Date Time: {start_date} {start_time} Check on RemoteCollab !'
                        email_from = settings.EMAIL_HOST_USER
                        recipient_list = [s_email]
                        send_mail(subject, message, email_from, recipient_list)
...
def create_task(request):
    ...
    if request.method == 'POST':
        ...
        if taskObj:
            ...
            subject = 'RemoteCollab - New Task Created for you'
            message = f'Hi {empDetails[0]["e_name"]} , Your organization as created a new task : {t_name} , description : {t_desc}, priority : {t_priority} and deadline for task is : {t_deadline_date}, Login in your account to get more information. From: RemoteCollab. '
            email_from = settings.EMAIL_HOST_USER
...
def update_task(request, pk):
    try:
        ...
        if request.method == 'POST':
            ...
            if tasks:
                empDetails = Employee.objects.filter(id=e_id, o_id_id=o_id).values()
                subject = 'RemoteCollab - Task Updated for you'
                message = f'Hi {empDetails[0]["e_name"]} , Your organization as updated a task : {t_name} , description : {t_desc}, priority : {t_priority} and deadline for task is : {t_deadline_date}, Login in your account to get more information. From: RemoteCollab. '
                email_from = settings.EMAIL_HOST_USER
                recipient_list = [empDetails[0]["e_email"], ]
                send_mail(subject, message, email_from, recipient_list)
...
def create_notice(request):
    ...
    if request.method == 'POST':
        ...
        if noticeObj:
            ...
            if user_details:
                for ud in user_details:
                    subject = 'RemoteCollab - New Notice Published'
                    s_name = ud['e_name']
                    s_email = ud['e_email']
                    message = f'Hi, {s_name} Your Organization has published a new notice, Notice Title: {on_title} Check on RemoteCollab !'
                    email_from = settings.EMAIL_HOST_USER
                    recipient_list = [s_email]
                    send_mail(subject, message, email_from, recipient_list)
